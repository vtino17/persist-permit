...
const assertProposal = (value: unknown): void => {
  ...
  if (!object(value.source) || typeof value.source.kind !== "string" || typeof value.source.origin !== "string" || typeof value.source.trust !== "string") {
...
export function assertPolicy(value: unknown): asserts value is RetentionPolicy {
  ...
  if (!strings(value.blockedSensitivities) || typeof value.rejectInstructionFromUntrusted !== "boolean") throw new Error("Policy controls are invalid.");
